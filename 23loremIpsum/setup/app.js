...
form.addEventListener('submit', function(e){
  e.preventDefault();
  const amtVal = parseInt(amount.value);
  const getRandom = Math.floor(Math.random()* text.length)
  // if empty , negative, morethan 9
  if(isNaN(amtVal) || amtVal <=0 || amtVal > 10){
    result.innerHTML = `<p class='result'>${text[getRandom]}</p>`;  
  }
  else {
    var  tempText= text.slice(0,amtVal); // returns list
    // console.log(tempText); // if amount of val is 2, then tempText will have two items of text arr
    tempText = tempText.map(function(singleParagraph){
      return `<p class="result">${singleParagraph}</p>`
    });
